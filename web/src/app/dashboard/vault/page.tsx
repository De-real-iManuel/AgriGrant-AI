    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-end gap-3">
        <button
          onClick={fetchVault}
          disabled={isLoading}
          className="flex items-center gap-2 px-3 py-2 rounded-lg border text-sm font-medium transition-all hover:bg-muted self-end"
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs mt-0.5" style={{ color: 'var(--muted-foreground)' }}>
                {missingCount > 0
                  ? `Upload ${missingCount} more doc${missingCount !== 1 ? 's' : ''} to improve`
      ...
      {error && (
        <div
          ...
          style={{ backgroundColor: 'var(--tint-red)', borderColor: 'rgba(220,38,38,0.2)' }}
        >
                  ...
                  >
                    ...
                    {doc.status === 'MISSING' ? (
                      ...
                      <a
                        ...
                        className="w-full py-2.5 rounded-xl border flex items-center justify-center gap-2 text-sm font-semibold transition-colors hover:bg-muted"
                        style={{ borderColor: 'var(--border)', color: 'var(--muted-foreground)' }}
                      >
                      ...
                      <div
                        className="py-2.5 rounded-xl text-center text-sm"
                        style={{
                          color: 'var(--muted-foreground)',
                          backgroundColor: 'var(--muted)',
                        }}
              ...
              >
                ...
                {vault?.documents.filter((d) => d.status !== 'MISSING').length === 0 ? (
                  <p
                    ...
                    style={{ color: 'var(--muted-foreground)' }}
                  >
                    No verification events yet. Upload a document to begin.
                  </p>
                ) : (
                  vault?.documents
                      ...
                      >
                        ...
                        <div>
                          <p className="text-sm font-medium" style={{ color: 'var(--foreground)' }}>
                            {doc.name} — {doc.status === 'VERIFIED' ? 'Verified' : 'Under Review'}
                          </p>
                          <p className="text-xs" style={{ color: 'var(--muted-foreground)' }}>
                            Securely stored in encrypted cloud storage
                          </p>
                        </div>
                      </div>
                    ))
              ...
              {vault?.hasBVN && (
                ...
                >
                  ...
                  <p className="text-xs font-medium" style={{ color: 'var(--primary)' }}>
                    BVN confirmed — +5 bonus trust points applied
                  </p>
                </div>
              )}
            </div>
...
export default function TrustVaultPage() {
  return (
    <AuthGuard>
      <DashboardShell
        title="Trust Vault"
        subtitle="Your verification documents and trust score — shared automatically with grantors."
      >
        <TrustVaultContent />
      </DashboardShell>
    </AuthGuard>
